<template>
  <div class="main-content">
      ...
      <el-form :inline="true" :model="searchForm" class="form-content">
        ...
        >
          <el-form-item :label="contents.inputTitle == 1 ? '设备编号' : ''">
            <el-input
              ...
              v-model="searchForm.shebeibianhao"
              placeholder="设备编号"
              clearable
            ></el-input>
            <el-input
        ...
        >
          <el-form-item>
            ...
            <el-button
              v-if="
                isAuth('shebeijieyong', '新增') && contents.btnAdAllIcon == 0
              "
              type="success"
              @click="addOrUpdateHandler()"
              >{{ contents.btnAdAllFont == 1 ? "新增" : "" }}</el-button
        ...
        >
          ...
          >
            <template slot-scope="scope">
              ...
              <el-button
                v-if="
                  isAuth('shebeijieyong', '查看') && contents.tableBtnIcon == 0
                "
                type="success"
                size="mini"
                @click="addOrUpdateHandler(scope.row.id, 'info')"
              ...
              <el-button
                ...
                size="mini"
                @click="
                  weixiushenbaoCrossAddOrUpdateHandler(scope.row, 'cross')
                "
                >{{ contents.tableBtnFont == 1 ? "申报" : "" }}</el-button
              >
              <el-button
                v-if="
                  isAuth('shebeijieyong', '申报') &&
              ...
              <el-button
                v-if="
                  isAuth('shebeijieyong', '修改') && contents.tableBtnIcon == 0
                "
                type="primary"
                size="mini"
                @click="addOrUpdateHandler(scope.row)"
              ...
              <el-button
                v-if="
                  isAuth('shebeijieyong', '删除') && contents.tableBtnIcon == 0
                "
                type="danger"
                size="mini"
                @click="deleteHandler(scope.row.id)"
    ...
    <weixiushenbao-cross-add-or-update
      v-if="weixiushenbaoCrossAddOrUpdateFlag"
      :parent="this"
      ref="weixiushenbaoCrossaddOrUpdate"
    ></weixiushenbao-cross-add-or-update>
...
import weixiushenbaoCrossAddOrUpdate from "../weixiushenbao/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      weixiushenbaoCrossAddOrUpdateFlag: false,
      contents: {
  ...
  methods: {
    ...
    weixiushenbaoCrossAddOrUpdateHandler(row, type) {
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.weixiushenbaoCrossAddOrUpdateFlag = true;
      console.log("点击详情11111",row);
      this.$storage.set("crossObj", row);
      this.$storage.set("crossTable", "shebeijieyong");
      this.$nextTick(() => {
        this.$refs.weixiushenbaoCrossaddOrUpdate.init(row.id, type);
      });
    },
    init() {},
    search() {
    ...
    getDataList() {
      ...
      if (
        ...
        this.searchForm.shebeibianhao != undefined
      ) {
    ...
    addOrUpdateHandler(row, type) {
      ...
      this.$nextTick(() => {
        this.$refs.addOrUpdate.init(row.id, type);
      });
    },
    // 查看评论
    // 审核窗口
    shDialog(row) {
      ...
      if (row) {
        this.shForm = {
          ...
          tupian: row.tupian,
          shengchanshang: row.shengchanshang,
          shuliang: row.shuliang,
          jieyongtianshu: row.jieyongtianshu,
          jieyongriqi: row.jieyongriqi,
          jieyongbeizhu: row.jieyongbeizhu,
