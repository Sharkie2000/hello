<template>
  <div class="main-content">
      ...
      <el-form :inline="true" :model="searchForm" class="form-content">
        ...
        >
          <el-form-item>
            ...
            <el-button
              v-if="
                isAuth('shebeirenling', '新增') && contents.btnAdAllIcon == 0
              "
              type="success"
              @click="addOrUpdateHandler()"
              >{{ contents.btnAdAllFont == 1 ? "新增" : "" }}</el-button
            ...
            <el-button
              v-if="
                isAuth('shebeirenling', '报表') && contents.btnAdAllIcon == 0
              "
              type="warning"
              @click="chartDialog()"
              >{{ contents.btnAdAllFont == 1 ? "统计报表" : "" }}</el-button
        ...
        >
          ...
          >
            <template slot-scope="scope">
              ...
              <el-button
                v-if="
                  isAuth('shebeirenling', '查看') && contents.tableBtnIcon == 0
                "
                type="success"
                size="mini"
                @click="addOrUpdateHandler(scope.row.id, 'info')"
              ...
              <el-button
                v-if="
                  isAuth('shebeirenling', '修改') && contents.tableBtnIcon == 0
                "
                type="primary"
                size="mini"
                @click="addOrUpdateHandler(scope.row)"
              ...
              <el-button
                v-if="
                  isAuth('shebeirenling', '删除') && contents.tableBtnIcon == 0
                "
                type="danger"
                size="mini"
                @click="deleteHandler(scope.row.id)"
    ...
    <el-dialog title="统计报表" :visible.sync="chartVisiable" width="800">
      <div id="zhejiulvChart" style="width: 100%; height: 600px"></div>
      <span slot="footer" class="dialog-footer">
...
export default {
  ...
  methods: {
    ...
    chartDialog() {
      ...
      this.$nextTick(() => {
        var zhejiulvChart = this.$echarts.init(
          document.getElementById("zhejiulvChart"),
          "macarons"
        );
        this.$http({
          url: "group/shebeirenling/zhejiulv",
          method: "get",
        }).then(({ data }) => {
          if (data && data.code === 0) {
            let res = data.data;
            ...
            for (let i = 0; i < res.length; i++) {
              ...
              // 使用刚指定的配置项和数据显示图表。
              zhejiulvChart.setOption(option);
              //根据窗口的大小变动图表
              window.onresize = function () {
                zhejiulvChart.resize();
              };
            }
          }
        });
    ...
    addOrUpdateHandler(row, type) {
      ...
      this.$storage.set("crossTable", "shebeirenling");
      this.showFlag = false;
